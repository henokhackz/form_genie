...
function SideNav() {
  ...
  return (
    <div className='h-screen shadow rounded-2xl'>
        <div className='p-5'>
            {menuList.map((menu,index)=>(
                <Link href={menu.path}  key={index} 
                className={`flex items-center gap-3 p-4 mb-3 
                hover:bg-neomorphic-primary hover:text-white rounded-lg
                cursor-pointer text-gray-500
                ${path==menu.path&&'bg-neomorphic-primary text-white'}
                `}>
                    <menu.icon/>
                    {menu.name}
                </Link>
        ...
        <div className='absolute bottom-[10%] p-6 w-64 mt-5 '>
            <Button className="w-full bg-neomorphic-primary text-white">+ Create Form</Button>
            <div className='my-7'>
